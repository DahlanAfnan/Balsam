...
$lessor = json_decode(file_get_contents('lessor.json'), true);
...
<head>
    <meta charset="UTF-8">
    <title><?php echo $xml->webpages->header->title; ?> - Rent</title>
...
<body>
    ...
    <div class="containers-products">
        <?php if (!empty($lessor)): ?>
            <?php foreach ($lessor as $lessor): ?>
                <div class="cards">
                    <a href="product_details.php?type=lessor&id=<?php echo $lessor['productId']; ?>">
                        <img src="<?php echo $lessor['productImage']; ?>" alt="<?php echo $lessor['productName']; ?> Image" style="width:400px;height:400px;">
                        <h2 class="card-title-product"><?php echo htmlspecialchars($lessor['productName']); ?></h2>
                        <p>SAR <?php echo htmlspecialchars($lessor['productPrice']); ?></p>
                    </a>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
